...
function ActiveCard() {
  ...
  const callApiUpdateCard = async (updateData) => {
    ...
    // Cập nhật lại card mới vào trong store để đồng bộ dữ liệu
    dispatch(updateCurrentActiveCard(updatedCard))

    // cập nhật card trong activeBoard (nested data) để đồng bộ dữ liệu board > columns > cards
          ...
          <Grid xs={12} sm={3}>
            ...
            <Stack direction="column" spacing={1}>
              ...
              {activeCard?.memberIds?.includes(currentUser._id) ? (
                <SidebarItem className="active" onClick={() => onUpdateCardMembers({ userId: currentUser._id, action: CARD_MEMBER_ACTIONS.REMOVE })}>
