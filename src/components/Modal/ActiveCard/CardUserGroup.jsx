...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  const FE_CardMembers = useMemo(() => {
    return board?.FE_allUsers.reduce((acc, user) => ({ ...acc, [user._id]: user }), {})
  }, [board?.FE_allUsers])
  ...
  return (
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      {cardMemberIds.map((id) =>
        <Tooltip title={FE_CardMembers[id]?.displayName} key={id}>
          <Avatar
            sx={{ width: 34, height: 34, cursor: 'pointer' }}
            alt={FE_CardMembers[id]?.displayName}
            src={FE_CardMembers[id]?.avatar}
          />
        </Tooltip>
      )}
