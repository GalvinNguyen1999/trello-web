...
function Notifications() {
  ...
  const [newNotification, setNewNotification] = useState(false) // Biến để check xem có thông báo mới hay không
  ...
  return (
    <Box>
      <Tooltip title="Notifications">
        <Badge
          color={newNotification ? 'warning' : 'default'}
      ...
      >
        {notifications.length === 0 && <MenuItem sx={{ minWidth: 200 }}>You do not have any new notifications.</MenuItem>}
            ...
            <MenuItem sx={{
              ...
              <Box sx={{ maxWidth: '100%', wordBreak: 'break-word', whiteSpace: 'pre-wrap', display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box><strong>{notification?.inviter?.displayName || notification?.inviter?.email}</strong> had invited you to join the board <strong>{notification?.board?.title}</strong></Box>
                </Box>
